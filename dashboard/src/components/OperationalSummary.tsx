...
export function OperationalSummary({ snapshot, hasError }: OperationalSummaryProps) {
  ...
  if (hasError) {
    ...
    summary = `A linha/EV ${snapshot.line_ev} possui ${snapshot.buses_found} veículo(s) detectado(s) no último ciclo. A melhor previsão atual é do veículo ${snapshot.best_prediction.vehicle_id ?? 'N/D'}, com ETA estimado em ${formatEta(snapshot.best_prediction.eta_min)}.`;
